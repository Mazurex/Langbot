...
class LanguagePaginator(discord.ui.View):
    ...
    def format_page(self):
        ...
        description = "\n".join(
            [f"**{code.upper()}** - {name}" for code, name in page_languages]
        )
        embed = discord.Embed(
...
class TranslateCmds(commands.Cog):
    ...
    @discord.app_commands.command(
        name="translate", description="Translate any text into any desired language"
    )
    @discord.app_commands.describe(text="The text to translate")
    @discord.app_commands.describe(
        target="The target language to translate to, run /supported to view valid languages. Defaults to the guilds target language"
    )
    async def translate(
        self, interaction: discord.Interaction, text: str, target: str | None = None
    ):
        ...
        if not is_valid:
            return await interaction.response.send_message(
                f"`{target}` is not a valid country code/name, run `/supported` to view all valid country codes",
                ephemeral=True,
            )
        ...
        try:
            ...
            await interaction.response.send_message(
                "There was an error with this command!", ephemeral=True
            )
    ...
    @discord.app_commands.command(
        name="supported",
        description="View a paginated embed of all supported languages",
    )
