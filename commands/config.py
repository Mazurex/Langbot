...
class Config(commands.Cog):
    def __init__(self, bot):
        ...
        self.config.command(
            name="view", description="View the server's LangBot configurations"
        )(self.view)
        self.config.command(
            name="reset", description="Reset all configurations to their default values"
        )(self.reset)
        self.config.command(name="translation-reply-message", description=description)(
            self.translation_reply_message
        )
        self.config.command(name="target-language", description=description)(
            self.target_lang
        )
        self.config.command(name="ignore-languages", description=description)(
            self.ignore_langs
        )
        self.config.command(name="ignore-bots", description=description)(
            self.ignore_bots
        )
        self.config.command(name="ignored-terms", description=description)(
            self.ignored_terms
        )
        self.config.command(name="reply", description=description)(self.reply)
        self.config.command(name="blacklisted-roles", description=description)(
            self.blacklisted_roles
        )
        self.config.command(name="auto-translate", description=description)(
            self.auto_translate
        )
        # ...
    ...
    async def view(self, interaction: discord.Interaction):
        ...
        **Auto Translate**
        ...
        if len(db_config["IGNORE_LANGS"]) > 0:
            ignore_languages = ", ".join(
                [
                    LANGUAGES.get(ignore_lang, "").capitalize()
                    for ignore_lang in db_config["IGNORE_LANGS"]
                ]
            )
        ...
        if len(db_config["BLACKLISTED_ROLES"]) > 0:
            blacklisted_roles = ", ".join(
                f"<@&{value}>" for value in db_config["BLACKLISTED_ROLES"]
            )

        # Add all config options as fields in the embed
        embed.add_field(
            name="Translation Reply Message",
            value=f"`{db_config['TRANSLATE_REPLY_MESSAGE']}`",
            inline=False,
        )
        embed.add_field(
            name="Target Language",
            value=LANGUAGES[db_config["TARGET_LANG"]].capitalize(),
            inline=False,
        )
        embed.add_field(name="Ignore Languages", value=ignore_languages, inline=False)
        embed.add_field(
            name="Ignore Bots",
            value="Yes" if db_config["IGNORE_BOTS"] else "No",
            inline=False,
        )
        embed.add_field(name="Ignored Terms", value=ignored_terms, inline=False)
        embed.add_field(
            name="Reply", value="Yes" if db_config["REPLY"] else "No", inline=False
        )
        embed.add_field(name="Blacklisted Roles", value=blacklisted_roles)
        embed.add_field(
            name="Auto Translate", value=db_config["AUTO_TRANSLATE"], inline=False
        )
    ...
    async def reset(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            "I have reset all LangBot configurations for this guild!", ephemeral=True
        )
        internal_print_log_message(interaction, "config/reset")

    async def translation_reply_message(
        self, interaction: discord.Interaction, value: str | None = None
    ):
        await interaction.response.defer(ephemeral=True)
        if value is None:
            ...
            embed.add_field(
                name="`{flag}`",
                value="The flag of the untranslated languages country",
                inline=False,
            )
            embed.add_field(
                name="`{translated}`", value="The translated message", inline=False
            )
            embed.add_field(
                name="`{original}`",
                value="The original untranslated message",
                inline=False,
            )
            embed.add_field(
                name="`{author_id}`",
                value="The ID of the author of the message",
                inline=False,
            )
            embed.add_field(
                name="`{guild_id}`",
                value="The ID of the guild of the message",
                inline=False,
            )
            embed.add_field(
                name="`{lang_code}`",
                value="The language code of the untranslated message",
                inline=False,
            )
            embed.add_field(
                name="`{lang_name}`",
                value="The language name of the untranslated message",
                inline=False,
            )
            ...
            await interaction.followup.send(
                f'Successfully updated "Translate Reply Message" to `{value}`',
                ephemeral=True,
            )
    ...
    async def target_lang(
        ...
        try:
            ...
            if value is None:
                ...
                await interaction.followup.send(
                    f'Successfully updated "Target Language" to `{LANGUAGES[value].capitalize()}`',
                    ephemeral=True,
                )
    ...
    async def ignore_langs(
        ...
        if len(config["IGNORE_LANGS"]) > 0:
            ignored_languages_str = ", ".join(
                LANGUAGES[i].capitalize() for i in config["IGNORE_LANGS"]
            )
        ...
        if not value:
            ...
            await interaction.followup.send(
                f'Successfully updated "Ignored Languaes" to `{", ".join(value)}`',
                ephemeral=True,
            )

    async def ignore_bots(
        self, interaction: discord.Interaction, value: bool | None = None
    ):
        await interaction.response.defer(ephemeral=True)
        ...
        if value is None:
            ...
            # The embed to reply with
            embed = discord.Embed(
                title="Ignore Bots", description=description, color=discord.Color.blue()
            )
            ...
            await interaction.followup.send(
                f'Successfully updated "Ignore Bots" to `{"Yes" if value else "No"}`',
                ephemeral=True,
            )
    ...
    async def ignored_terms(
        ...
        if value is None:
            ...
            if not value:
                await interaction.followup.send(
                    f'Successfully disabled "Ignord Terms"', ephemeral=True
                )
            else:
                await interaction.followup.send(
                    f'Successfully updated "Ignored Terms" to `{", ".join(value)}`',
                    ephemeral=True,
                )

    async def reply(self, interaction: discord.Interaction, value: bool | None = None):
        await interaction.response.defer(ephemeral=True)
        ...
        if value is None:
            ...
            # The embed to reply with
            embed = discord.Embed(
                title="Reply", description=description, color=discord.Color.blue()
            )
            ...
            await interaction.followup.send(
                f'Successfully updated "Reply" to `{"Yes" if value else "No"}`',
                ephemeral=True,
            )
    ...
    async def blacklisted_roles(
        ...
        if result is False:
            return await interaction.followup.send(
                f"All roles given were invalid, are they correct? Are they in this guild?",
                ephemeral=True,
            )
        if len(value) == 0:  # type: ignore
            return await interaction.followup.send(
                f'Successfully set "Blacklisted Roles" to `nothing`'
            )
    ...
    async def auto_translate(
        ...
        if value is None:
            ...
            await interaction.followup.send(
                f'Successfully updated "Auto Translate" to `{"Yes" if value else "No"}`',
                ephemeral=True,
            )
